...
// Behavior preserved across the predictionService extraction (Track 2 commit 1):
//   source=all_list_items, minOccurrences=2, urgencyThreshold=0.6,
//   limit=8 (matches the original SQL LIMIT 8), then slice to 6 in the
//   response. Message uses the pre-slice count, just like the original.
router.get('/smart-suggestions', authenticate, async (req, res) => {
  try {
    const all = await predictionService.getRestockSuggestions({
      userId: req.user.id,
    ...
    successResponse(res, {
      ...
      message: `${all.length} items may need restocking`,
    });
  } catch (error) {
